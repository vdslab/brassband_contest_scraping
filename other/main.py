...
for item in allData:
    ...
    if not s in allShortData:
        ...
        d = {"name": item["school"], "prize": item["prize"],
             "prefecture": prefecture, "last": last, "representative": item["representative"]}
...
for name in allShortData.keys():
    year = name[-4:]
    divideYearData[year].append(allShortData[name])


adjustedData = []

for year in divideYearData:
    print(year)
    adjustedData.append({"year": year, "data": divideYearData[year]})

with open("allShortData.json", "w") as f:
    json.dump(adjustedData, f, indent=2, ensure_ascii=False)
